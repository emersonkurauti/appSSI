...
set _DeploySetParametersFile=%RootPath%wappSSI.SetParameters.xml
)
...
@rem ---------------------------------------------------------------------------------
if not exist "%RootPath%wappSSI.zip" (
echo "%RootPath%wappSSI.zip" does not exist. 
...
echo. "%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%wappSSI.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\Release\\Package\\PackageTmp\\App_Data$' %_MsDeployAdditionalFlags%
"%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%wappSSI.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\Release\\Package\\PackageTmp\\App_Data$' %_MsDeployAdditionalFlags%
) else (
echo. "%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%wappSSI.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\Release\\Package\\PackageTmp\\App_Data$' -setParamFile:"%RootPath%wappSSI.SetParameters.xml" %_MsDeployAdditionalFlags%
...
:usage
echo =========================================================
if not exist "%RootPath%wappSSI.deploy-readme.txt" (
...
start notepad "%RootPath%wappSSI.deploy-readme.txt"
)
echo =========================================================
goto :eof
...
:CheckParameterFile
echo =========================================================
if  exist "%RootPath%wappSSI.SetParameters.xml" (
echo SetParameters from:
echo "%RootPath%wappSSI.SetParameters.xml"
echo You can change IIS Application Name, Physical path, connectionString
echo or other deploy parameters in the above file.
) else (
echo SetParamterFiles does not exist in package location.
